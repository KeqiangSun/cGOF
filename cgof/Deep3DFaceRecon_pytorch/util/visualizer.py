...
class MyVisualizer:
    def __init__(self, opt):
        ...
        self.name = opt.name
        self.img_dir = os.path.join(opt.img_folder, 'face_recon')
        ...
        for label, image in visuals.items():
            for i in range(image.shape[0]):
                ...
                if save_results:
                    ...
                    if name is not None:
                        img_path = os.path.join(save_path, '%s_%s.png' % (name, label))
                    else:
                        img_path = os.path.join(save_path, '%s_%03d.png' % (label, i + count))
                    # print(f"saving to: {img_path}")
                    util.save_image(image_numpy, img_path)

    def tb_current_results(self, tb_writer, visuals, global_step):
        """Display current results on tensorboad; save current results to an HTML file.

        Parameters:
            tb_writer (summary writer) - - tensorboard summary writer
            visuals (OrderedDict) - - dictionary of images to display or save
            total_iters (int) -- total iterations
            epoch (int) - - the current epoch
            dataset (str) - - 'train' or 'val' or 'test'
        ...
        for label, image in visuals.items():
            ...
            tb_writer.add_image(label, image_numpy,
