...
def parse():
    ...
    parser.add_argument('--factor', type=int, default=1, help='factor variation mode. 0 = id, 1 = expression, 2 = lighting, 3 = pose, 4 = all.')
    parser.add_argument('--recon_data_root', type=str, default="imgs/stylerig_pie/gen/face_recon/pie/exp/epoch_20_000000/", help='where to find the reconstructed results.')
...
def main():
    ...
    if factor == 0:
        if "/pie/" in data_root:
            seps = ["_x_0", "_x_1", "_y_0", "_y_1",  "(1)"]
        elif "/stylerig/" in data_root:
            seps = ["_x_0", "_x_1", "_y_0", "_y_1", ""]

    for k in data_dict.keys():
        for s in seps:
            if s in k and len(s) > 0:
                n = k.split(s)[0]
                names.append(n)
    names = set(names)
    ...
    std_angles = []
    for sep in seps:
        variations = [name+s for name in names]
        ids = []
        exps = []
        angles = []
        for v in variations:
    ...
    print(f"processing folder: {args.recon_data_root}, factor:{factor2key[factor]}")
