...
class RGBDAdapterBlock(nn.Module):
    def __init__(self, output_channels):
        super().__init__()
        self.model = nn.Sequential(
            nn.Conv2d(4, output_channels, 1, padding=0),
            nn.LeakyReLU(0.2, inplace=True)
        )
    def forward(self, input):
        return self.model(input)
...
class CCSEncoderDiscriminator(nn.Module):
    ...
    def forward(self, input, alpha, options=None, **kwargs):
        ...
        prediction = x[..., 0:1]
        latent = x[..., 1:(1+self.latent_dim)]
        position = x[..., (1+self.latent_dim):]

        return prediction, latent, position

class RGBDEncoderDiscriminator(nn.Module):
    def __init__(self, **kwargs): # from 4 * 2^0 to 4 * 2^7 4 -> 512
        super().__init__()
        self.latent_dim = kwargs.get('latent_dim', 256)
        self.epoch = 0
        ...
        [
            RGBDAdapterBlock(32),
            RGBDAdapterBlock(64),
            RGBDAdapterBlock(128),
            RGBDAdapterBlock(256),
            RGBDAdapterBlock(400),
            RGBDAdapterBlock(400),
            RGBDAdapterBlock(400),
            RGBDAdapterBlock(400)
        ])
