...
"""python edit_latent_code.py --network ./inversion/outputs/embeddings/leonardo_dicaprio/PTI/leonardo_dicaprio/model_leonardo_dicaprio.pt --w_path ./inversion/outputs/embeddings/leonardo_dicaprio/PTI/leonardo_dicaprio/optimized_noise_dict.pickle --c_path ./proj_data/input/crop_1024/leonardo_dicaprio_c.npy --z_path ./proj_data/input/crop_1024/proj_data/input/crop_1024/epoch_20_000000/leonardo_dicaprio.mat --outdir ./edit --sample_mult 2 --use_face_recon True"""
import os
import re
...
def generate_images(
    ...
    with torch.no_grad():
        G.z = z
        frame = G.synthesis(latent, c, face_recon=None, noise_mode='const', force_fp32=True)[image_mode][0]
    ...
    Image.fromarray(synth_image, 'RGB').save(os.path.join(outdir, f"fit_pic.png"))
    ...
    with torch.no_grad():
        G.z = z[1:]
        frames = G.synthesis(
            edit_f, c.repeat([edit_f.shape[0], 1]), face_recon=None, noise_mode='const')[image_mode]
    for i in range(frames.shape[0]):
        # save_image(
        #     frames[i:i+1], os.path.join(outdir, f"edit_pic_{i}.png"), normalize=True)
        synth_image = frames[i:i+1]
        synth_image = (synth_image + 1) * (255/2)
        synth_image = synth_image.permute(0,2,3,1)[0].clamp(0, 255).to(torch.uint8).cpu().numpy()
        Image.fromarray(synth_image, 'RGB').save(os.path.join(outdir, f"edit_pic_{i}.png"))
    ...
    # G.z = z
    # gen_interp_video(G=G, face_recon=None, latent = latent,mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
    #                  w_frames=w_frames,  psi=truncation_psi,
    #                  truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode)
    # # gen_interp_video(G=G, face_recon=face_recon, latent = latent,mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
    # #                  w_frames=w_frames,  psi=truncation_psi,
    # #                  truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode)
