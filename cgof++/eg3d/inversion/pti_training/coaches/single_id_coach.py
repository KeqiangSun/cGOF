...
class SingleIDCoach(BaseCoach):
    ...
    def __init__(self, data_loader, use_wandb, reload_modules=False, use_face_recon=False):
        super().__init__(data_loader, use_wandb, reload_modules, use_face_recon)
    ...
    def train(self):
        ...
        for idx, (fname, image) in enumerate(tqdm(self.data_loader)):
            ...
            print(f"embedding_fir: {embedding_dir}")

            w_pivot = None

            # import cv2
            # mask = cv2.imread('/home/kqsun/Tasks/eg3d/eg3d/face-parsing.PyTorch/res/test_res/bieber.png')
            # mask = mask/mask.max()
            # mask = torch.from_numpy(mask.transpose(2,0,1)).to(global_config.device)
            # mask = torch.ones_like(mask)
            mask = None

            if hyperparameters.use_last_w_pivots:
                # w_pivot = self.load_inversions(w_path_dir, image_name)
                with open(f'{embedding_dir}/optimized_noise_dict.pickle', 'rb') as handle:
                    optimized_dict = pickle.load(handle)
                    w_pivot = torch.from_numpy(optimized_dict['projected_w']).to(global_config.device)
                    all_w_opt = torch.from_numpy(optimized_dict['all_w_opt']).to(global_config.device)
                    noise_bufs = optimized_dict['noise_bufs']
                    for noise_buf in noise_bufs:
                        noise_bufs[noise_buf] = torch.from_numpy(noise_bufs[noise_buf]).to(global_config.device)
            elif not hyperparameters.use_last_w_pivots or w_pivot is None:
                w_pivot, noise_bufs, all_w_opt = self.calc_inversions(image, image_name, embedding_dir, write_video=True, mask=mask)
            ...
            for i in tqdm(range(hyperparameters.max_pti_steps)):
                ...
                if mask is not None:
                    ...
                    generated_images = generated_images * mask
                    real_images_batch = real_images_batch * mask
            ...
            print(f"saving to: {f'{embedding_dir}/model_{image_name}.pt'}")
