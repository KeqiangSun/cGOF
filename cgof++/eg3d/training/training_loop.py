...
def training_loop(
    ...
    #---------------------------------------------------------------
    # Modified by Sun et al.
    ctrl_kwargs             = {},       # Options for controlment.
    #---------------------------------------------------------------
):
    # Initialize.
    ...
    # Modified by Sun et al.
    if rank == 0:
        print('Building Face Recon networks...')
    face_recon, _ = init_face_recon(device)
    # G_kwargs.update({"face_recon":face_recon})
    #---------------------------------------------------------------
    ...
    #---------------------------------------------------------------
    # Modified by Sun et al.
    loss = dnnlib.util.construct_class_by_name(device=device, G=G, D=D, face_recon=face_recon, augment_pipe=augment_pipe, ctrl_kwargs=ctrl_kwargs, **loss_kwargs) # subclass of training.loss.Loss
    #---------------------------------------------------------------
    ...
    while True:
        ...
        # loss.ctrl_kwargs.cur_tick = cur_tick
        G.rendering_kwargs['cur_tick'] = cur_tick
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg, verbose=(rank == 0) and (image_snapshot_ticks is not None) and (cur_tick % image_snapshot_ticks == 0))
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            #---------------------------------------------------------------
            # Modified by Sun et al.
            out = [G_ema(z=z, c=c, face_recon=face_recon, noise_mode='const') for z, c in zip(grid_z, grid_c)]
            if ctrl_kwargs.using_warping_loss:
                cv2.imwrite(os.path.join(run_dir, f'warping{cur_nimg//1000:06d}.png'), loss.exp_warping_img)
            #---------------------------------------------------------------
