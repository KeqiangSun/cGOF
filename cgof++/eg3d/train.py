...
@click.option('--recon_lambda', help='recon loss weight', metavar='FLOAT',              type=click.FloatRange(min=0), default=4.0, show_default=True)
@click.option('--sample_near_mesh', help='sample near mesh', metavar='BOOL',            type=bool, default=False, show_default=True)
@click.option('--thick_ratio',  help='thick ratio of mesh region', metavar='FLOAT',     type=click.FloatRange(min=0), default=0.005, show_default=True)
@click.option('--shrink_step_num', help='step num required to shrink', metavar='FLOAT', type=click.FloatRange(min=0), default=200.0, show_default=True)
@click.option('--dist_depr_lambda', help='dist_depr weight', metavar='FLOAT',           type=click.FloatRange(min=0), default=10000.0, show_default=True)
@click.option('--bg_depr_lambda', help='bg_depr weight', metavar='FLOAT',               type=click.FloatRange(min=0), default=10000.0, show_default=True)
@click.option('--norm_reg_lambda', help='recon loss weight', metavar='FLOAT',           type=click.FloatRange(min=0), default=1000.0, show_default=True)
@click.option('--geo_reg_lambda', help='recon loss weight', metavar='FLOAT',            type=click.FloatRange(min=0), default=500.0, show_default=True)
@click.option('--exp_warping_lambda', help='recon loss weight', metavar='FLOAT',        type=click.FloatRange(min=0), default=10.0, show_default=True)
@click.option('--lm3d_lambda',  help='recon loss weight', metavar='FLOAT',              type=click.FloatRange(min=0), default=300.0, show_default=True)
#---------------------------------------------------------------
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=411, w_dim=512, mapping_kwargs=dnnlib.EasyDict())
    ...
    c.image_snapshot_ticks = c.network_snapshot_ticks = opts.snap if opts.metrics else None
    # c.image_snapshot_ticks = c.network_snapshot_ticks = None
    c.random_seed = c.training_set_kwargs.random_seed = opts.seed
    c.data_loader_kwargs.num_workers = opts.workers

    #---------------------------------------------------------------
    # Modified by Sun et al.
    c.ctrl_kwargs = dnnlib.EasyDict()
    c.ctrl_kwargs.recon_lambda = opts.recon_lambda
    c.ctrl_kwargs.sample_near_mesh = opts.sample_near_mesh
    c.ctrl_kwargs.thick_ratio = opts.thick_ratio
    c.ctrl_kwargs.shrink_step_num = opts.shrink_step_num
    c.ctrl_kwargs.dist_depr_lambda = opts.dist_depr_lambda
    c.ctrl_kwargs.bg_depr_lambda = opts.bg_depr_lambda
    c.ctrl_kwargs.norm_reg_lambda = opts.norm_reg_lambda
    c.ctrl_kwargs.geo_reg_lambda = opts.geo_reg_lambda
    c.ctrl_kwargs.exp_warping_lambda = opts.exp_warping_lambda
    c.ctrl_kwargs.lm3d_lambda = opts.lm3d_lambda

    c.ctrl_kwargs.using_recon_loss = c.ctrl_kwargs.recon_lambda > 0
    c.ctrl_kwargs.using_ldmk_loss = c.ctrl_kwargs.lm3d_lambda > 0
    c.ctrl_kwargs.using_warping_loss = opts.exp_warping_lambda > 0
    c.ctrl_kwargs.using_dist_depr = opts.dist_depr_lambda > 0
    c.ctrl_kwargs.using_geo_reg = opts.geo_reg_lambda > 0
    #---------------------------------------------------------------
    ...
    #---------------------------------------------------------------
    # Modified by Sun et al.
    c.G_kwargs.rendering_kwargs.update({"sample_near_mesh": opts.sample_near_mesh})
    c.G_kwargs.rendering_kwargs.update({"thick_ratio": opts.thick_ratio})
    c.G_kwargs.rendering_kwargs.update({"shrink_step_num": opts.shrink_step_num})
    c.G_kwargs.rendering_kwargs.update({"using_dist_depr": opts.dist_depr_lambda > 0})
    #---------------------------------------------------------------
